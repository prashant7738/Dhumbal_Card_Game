...
void bot::botthrow() {
    if(findconsecutivecardsamesuit() && (botselectedcard.size() >= 4 || (botselectedcard[0].value >> 1)) ) {
        botthrowncard=botselectedcard;
        botselectedcard.clear();
        arrange();
    }
    else if(findsamevaluecarddiffsuit() && (botselectedcard.size() >= 3 || (botselectedcard[0].value >> 2))) {
        botthrowncard=botselectedcard;
        botselectedcard.clear();
        arrange();
    }
